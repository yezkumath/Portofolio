...
{
    public partial class Rekapitulasi_BAP : Form
    {
        clsCon conn = new clsCon();
        public string tempBulan, tempTahun, tempNamaSupp, tempKodeNamaSupp, tempKodeDC, tempNamaDC, user;

        public Rekapitulasi_BAP(string bulan, string tahun, string namaSupp, string tampungKodeNamaSupp, string kodedc, string namadc, string username)
        {
            InitializeComponent();
            tempBulan = bulan;
            tempTahun = tahun;
        ...
        private void Rekapitulasi_BAP_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (tempNamaSupp != "ALL")
                {
                    cmd.CommandText = clsSQL.Rekapitulasi_BAP_PerSUP(tempBulan, tempTahun, tempNamaSupp);
                }
                else
                {
                    cmd.CommandText = clsSQL.Rekapitulasi_BAP_ALL(tempBulan, tempTahun);
                    tempKodeNamaSupp = " ";
                }
                ...
                this.reportViewer1.LocalReport.DataSources.Add(new ReportDataSource("DT_LaporanBAP", DT));
